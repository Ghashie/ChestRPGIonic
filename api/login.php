...
header("Access-Control-Allow-Credentials: true"); // Adiciona esta linha
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (!$idUsuario) {
        error_log("ID de Usuário não encontrado na sessão");
    ...
    if (!$idAdmin) {
        error_log("ID de Admin não encontrado na sessão");
        http_response_code(401);
        exit(json_encode(['error' => 'Usuário não autenticado']));
    }
    ...
    if ($stmt->execute()) {
        ...
        error_log("Erro ao criar a mesa");
    ...
    if (isset($putData['idTable'])) {
        ...
        if ($stmt->execute()) {
            ...
            error_log("Erro ao atualizar a mesa");
    ...
    if (isset($_GET['idTable'])) {
        ...
        if ($sql) {
            ...
            error_log("Erro ao excluir a mesa");
            exit(json_encode(['status' => 'Deu ruim']));
        }
    }
} else {
